...
function extractCertificateData() {
  try {
    ...
    for (let i = 0; i < allCells.length; i++) {
      ...
      // Start Date
      if (cellText === 'Start Date:' && allCells[i + 1]) {
        data.startDate = allCells[i + 1].textContent.trim();
      }
      
      // End Date
      if (cellText === 'End Date:' && allCells[i + 1]) {
        data.endDate = allCells[i + 1].textContent.trim();
      }
      
      // Course
      if (cellText === 'Course:' && allCells[i + 1]) {
        data.course = allCells[i + 1].textContent.trim();
      }
    }
    ...
    allInputs.forEach(input => {
      ...
      // Course field - check by ID or name first
      if ((inputId.includes('course') || inputName.includes('course')) && value) {
        data.course = value;
      }
      ...
      // Start Date field - check by ID or name
      if ((inputId.includes('start') || inputName.includes('start')) && value) {
        data.startDate = value;
      }
      
      // End Date field - check by ID or name
      if ((inputId.includes('end') || inputName.includes('end')) && value) {
        data.endDate = value;
      }
    });
